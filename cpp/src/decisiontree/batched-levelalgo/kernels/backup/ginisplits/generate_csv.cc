...
// Function to generate random float between min and max, rounded to 2 decimals
float getRandomFloat(std::mt19937& gen, float min = 0.0f, float max = 10.0f) {
    std::uniform_real_distribution<float> dist(min, max);
    float raw = dist(gen);
    return std::round(raw * 100.0f) / 100.0f;
